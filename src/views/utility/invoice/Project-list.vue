    ...
    <Card noborder>
      <div class="md:flex justify-between pb-6 md:space-y-0 space-y-3 items-center">
        ...
        <div class="flex items-center space-x-3"> <!-- Contenedor flex para alinear los elementos -->
          ...
          <InputGroup v-model="searchTerm" placeholder="Buscar colaborador" type="text" prependIcon="heroicons-outline:search"
            merged />
        </div>
      </div>
      ...
      <EditProject :activeModal="showEditModal" @close="showEditModal = false" :colaboradorData="colaboradorData"
      title="Actualizar Registro de Colaborador" @updateColaboradorList="listarColaboradores"></EditProject>
...
import EditProject from "./CollaboratorEditModal";
...
export default {
        ...
        },
        {
          ...
          doit: async (data) => {
            try {
              ...
              await axios.delete(`${import.meta.env.VITE_API_URL}/collaborators/delete/${data.id}`, headers);
  ...
  methods: {
    async reloadCrmTable() {
      try {
        await this.listarColaboradores(); // Llama al método listarEmpleados()
