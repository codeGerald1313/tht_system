            ...
            <template v-slot:table-row="props">
              ...
              <span v-if="props.column.field == 'abierto'">
                <div class="flex space-x-3 items-center text-left rtl:space-x-reverse">
                  <Checkbox v-model="projects[props.index].esAbierto" :checked="projects[props.index].esAbierto" />
                </div>
              </span>
      ...
      <div class="grid grid-cols-1">
        <div class="flex flex-col">
          ...
          <vue-good-table class="-mx-6" :columns="columnsReservasAlojamiento"
            styleClass="vgt-table table-head v-middle striped listview" :rows="reservasHotel" :sort-options="{
              enabled: false,
            }">
            <template v-slot:table-row="props">
              ...
              <template v-if="props.column.field === 'action' && showDeleteButton">
...
<div class="grid grid-cols-1 lg:grid-cols-1 gap-5">
  ...
  <div class="grid grid-cols-3 gap-5">
    ...
    <EditProject2 :activeModal="showReserveModal" title="Actualizar Registro de Colaborador"
      @postComplete="handlePostComplete" :idCliente="idCliente" :code="code" @close="showReserveModal = false">
    </EditProject2>
...
export default {
  ...
  data() {
    return {
      ...
      showDeleteButton: true, // Variable de estado para controlar la visibilidad del botón de eliminación
  ...
  watch: {
    ...
    '$route': {
      ...
      handler(to, from) {
        ...
        this.booking.numChildren = otherData.nro_pax_childs;
        this.booking.numAdults = otherData.nro_pax_adults;
        this.booking.numSeniors = otherData.nro_pax_olds;
        this.booking.dateTimeArrival = otherData.date_arrival + ' ' + otherData.time_arrival;
        this.booking.dateArrival = otherData.date_arrival;
        this.booking.timeArrival = otherData.time_arrival;
        this.booking.arrivalBy = otherData.arrivalplace_id;
        this.booking.placeArrivalReference = otherData.arrival_place2;
        this.booking.voucherReference = otherData.reference_voucher;
        this.booking.emergencyContact = otherData.contact_emergency;
        this.booking.emergencyPhone = otherData.telephone_emergency;
        this.booking.observations = otherData.observations;
        this.booking.reference_location = otherData.reference_location;
        this.booking.isFavorite = otherData.is_favorite === 1 ? true : false;

        this.booking.includeBreakfast = otherData.is_breakfast === 1 ? true : false;

        this.booking.numBreakfasts = otherData.nro_breakfast;
        this.booking.startDate = otherData.start_breakfast;
        ...
        this.projects = otherData.tours.map(tour => ({
          id: tour.id,
          esAbierto: tour.pivot.is_open === 1 ? true : false,
          description: tour.description,
          vehiculo: tour.pivot.vehicle_tour,
          turno: tour.pivot.shift_tour,
          nPasajseros: tour.pivot.quantity,
          date_assigned: tour.pivot.date_assigned,
          observacion: tour.pivot.observation,
          realizado: tour.pivot.is_realized,
          precio_corporate: tour.price_corporate,
          cobranza: tour.pivot.delegated_payment,
        }));
  ...
  methods: {
    ...
    handlePostComplete(data) {
      ...
      // Verifica si los datos de la reserva de hotel son un objeto o un array
      if (Array.isArray(data.data)) {
        // Si es un array, iterar sobre cada elemento
        data.data.forEach(reserva => {
          // Agregar la reserva al array this.reservasHotel
          this.reservasHotel.push(this.formatReserva(reserva));

          // Agregar el total de la reserva al totalSum
          totalSum += parseFloat(reserva.total);

          // Agregar el ID de la reserva al array idsReservesHotels si no está presente
          const nuevoIdReserva = reserva.id;
          if (!this.idsReservesHotels.includes(nuevoIdReserva)) {
            this.idsReservesHotels.push(nuevoIdReserva);
          }
        });
      } else {
        // Si es un objeto, agregar la reserva única
        this.reservasHotel.push(this.formatReserva(data.data));

        // Agregar el total de la reserva al totalSum
        totalSum += parseFloat(data.data.total);

        // Agregar el ID de la reserva al array idsReservesHotels si no está presente
        const nuevoIdReserva = data.data.id;
        if (!this.idsReservesHotels.includes(nuevoIdReserva)) {
          this.idsReservesHotels.push(nuevoIdReserva);
        }
      }
    ...
    calcularTotalHotelValue() {
      ...
      // Iterar sobre cada reserva de hotel y sumar su total
      this.reservasHotel.forEach(reserva => {
        // Convertir el total a un número y sumarlo
    ...
    formatReserva(reserva) {
      return {
        code: reserva.code,
        hotel: {
          tradename: reserva.hotel?.tradename || '',
          citie_turistic: {
            description: reserva.hotel?.citie_turistic?.description || '',
          },
          cellphone: reserva.hotel?.cellphone || '',
        },
        detailbedrooms: reserva.typebedrooms.map(bedroom => ({
          quantity: bedroom.quantity,
          nights: bedroom.nights,
          description: bedroom.typebedroom?.description || '',
        })),
    ...
    saveReserva() {
      ...
      const dataToSend = {
        ...
        time_arrival: this.booking.timeArrival !== "null" ? this.booking.timeArrival : null,
    ...
    openModalReservaHotel() {
      ...
      this.showDeleteButton = false; // Desactiva el botón de eliminación al abrir el modal
    },
