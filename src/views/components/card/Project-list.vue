    ...
    <Card noborder>
      <div class="md:flex justify-between pb-6 md:space-y-0 space-y-3 items-center">
        ...
        <div class="flex items-center space-x-3"> <!-- Contenedor flex para alinear los elementos -->
          ...
          <InputGroup v-model="searchTerm" placeholder="Buscar transporte" type="text" prependIcon="heroicons-outline:search"
            merged />
        </div>
      </div>
      ...
      <vue-good-table class="-mx-6" :columns="columns" styleClass=" vgt-table  table-head   v-middle striped listview"
        ...
        <template v-slot:table-row="props">
          ...
          <span v-if="props.column.field == 'owner'">
            {{ props.row.owner }}
          
            <br/>
            N° Doc: {{ props.row.document }}

          </span>

          <span v-if="props.column.field == 'vehicle_type'">
            {{ props.row.vehicle_type }}
          
            <br/>
            N° placa: {{ props.row.license_plate }}
