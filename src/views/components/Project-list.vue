    ...
    <Card noborder>
      <div class="md:flex justify-between pb-6 md:space-y-0 space-y-3 items-center">
        ...
        <div class="flex items-center space-x-3"> <!-- Contenedor flex para alinear los elementos -->
          ...
          <InputGroup v-model="searchTerm" placeholder="Buscar Hotel" type="text" prependIcon="heroicons-outline:search"
            merged />
        </div>
      </div>
      ...
      <EditProject :activeModal="showEditModal" @close="showEditModal = false" :hotelData="hotelData"
        title="Actualizar Datos Empleado" @updateHotelList="listarHoteles"></EditProject>
  ...
  data() {
    return {
      ...
      actions: [
        {
          ...
          doit: (data) => {
            // console.log(data);
            this.hotelData = data; // Asigna los datos del empleado
      ...
      columns: [
        ...
        {
          label: "Ciudad",
          field: "citie_turistic.description",
        },
        {
          label: "Celular",
          field: "cellphone",
  ...
  methods: {
    ...
    async reloadCrmTable() {
      try {
        ...
        console.error('Error al recargar la tabla de hoteles:', error);
      }
    },
    ...
    async listarHoteles() {
      try {
        ...
        const toast = useToast();
        toast.success('Hoteles listados correctamente', {
          timeout: 1500, // Cierre automático después de 1.5 segundos
        });
