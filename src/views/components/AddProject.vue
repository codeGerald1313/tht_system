<template>
  <div>
    <Modal :activeModal="store.addmodal" @close="store.closeModal" title="Registrar nuevo Hotel" sizeClass="max-w-5xl"
      centered>
      <form @submit.prevent="saveHotel" class="space-y-4">
        ...
        <div class="grid lg:grid-cols-3 gap-4">
          <FromGroup label="Ciudad turística">
            <Select :options="cityOptions" ref="citySelect" v-model="selectedCity" placeholder="Seleccione ciudad" class="city-select" />
        ...
        <div class="grid lg:grid-cols-2 gap-4">
          ...
          <VueSelect label="Categoría" :options="categoryOptions" v-model="form.category" class="category-select">
          ...
          <Card bodyClass="p-0 mt-7">
            <header
              ...
              <div class="flex items-center"> <!-- Contenedor flex -->
                ...
                <span v-if="showError && !roomType" class="text-red-500 ml-2">Requerido</span>
              </div>
...
const saveHotel = () => {
  ...
  axios.post(`${import.meta.env.VITE_API_URL}/hotels/create-asing`, form.value, {
    ...headers
  })
    .then(response => {
...
const categoryOptions = ['Sin asignar', '1 estrella', '2 estrellas', '3 estrellas', '4 estrellas', '5 estrellas'];
