<template>
  <div>
    <Modal :activeModal="props.activeModal" @close="closeEditModal" title="Actualizar Datos Empleado"
        ...
        <form @submit.prevent="saveEmployee" class="space-y-4 col-span-3">
          <div class="grid lg:grid-cols-3 gap-4">
            ...
            <div class="flex items-center">
              <FromGroup label="Puesto de trabajo" class="flex-1">
                <Select :options="jobOptions" v-model="formData.jobSelected" class="w-full job-select" />
            ...
            <FromGroup label="Tipo documento" class="type-document-select">
              <Select :options="typeDocumentOptions" v-model="selected" disabled />
          ...
          <div class="grid lg:grid-cols-3 gap-4 grid-cols-1">
            ...
            <FromGroup label="Estado">
              <Select :options="statusOptions" v-model="formData.statusSelected" class="estado-select" />
            </FromGroup>
            <!-- Sexo -->
            <FromGroup label="Sexo">
              <Select :options="sexOptions" v-model="formData.sexSelected" class="sexo-select" />
            </FromGroup>
          </div>
        </form>
...
const props = defineProps({
  activeModal: Boolean,
  employeeData: Object // Se espera que la data del empleado venga como objeto
})

const emits = defineEmits(['close', 'updateEmployeeList']);
...
const selected = ref(typeDocumentOptions[0].label); // Inicializar con el valor de la primera opción
...
const saveEmployee = () => {
  ...
  const selectElementTypeDocument = document.querySelector('.type-document-select .vs__selected-options');
  processSelectedOption(selectElementTypeDocument, typeDocumentOptions, selectTipoDocumento, 'typeDocumentSelected');
  ...
  if (!formData.value.typeDocumentSelected) {
    document.getElementById('type-document-error').style.display = 'block';
    validationFailed = true;
  } else {
    document.getElementById('type-document-error').style.display = 'none';
  }
  ...
  formDataToSend.append('companyjob_id', formData.value.jobSelectedId);
...
const handleInput = () => {
  // Ocultar mensajes de error si los campos tienen algún valor
  if (formData.value.typeDocumentSelected) {
    document.getElementById('type-document-error').style.display = 'none';
  }
  if (formData.value.inputValue) {
...
watch(() => props.employeeData, (newData) => {
  if (newData) {
    ...
    formData.value.idEmploye = newData.id

    formData.value.inputValue = newData.document
    formData.value.clientFullname = newData.fullname
    formData.value.clientAddress = newData.address
    formData.value.clientCellphone = newData.cellphone
    formData.value.clientTelephone = newData.telephone
    formData.value.clientEmail = newData.email
    formData.value.dateBirthday = newData.date_birthday


    // Encontrar las opciones seleccionadas para sexo y estado
    const selectedSexOption = sexOptions.find(option => option.label === newData.sex);
    const selectedStatusOption = statusOptions.find(option => option.label === newData.status);

    // Asignar los valores seleccionados a formData
    if (selectedSexOption) {
      formData.value.sexSelected = selectedSexOption.label;
    }
    if (selectedStatusOption) {
      formData.value.statusSelected = selectedStatusOption.label;
    }


    const selectedJobOption = jobOptions.value.find(option => option.id === newData.companyjob_id);
    if (selectedJobOption) {
      formData.value.jobSelected = selectedJobOption.label;
      formData.value.jobSelectedId = selectedJobOption.id;
    } else {
      // Si no se encuentra el trabajo seleccionado en las opciones disponibles, limpiar los valores
      formData.value.jobSelected = '';
      formData.value.jobSelectedId = '';
    }
  }
});

// Escuchar cambios en formData.jobSelected y actualizar formData.jobSelectedId
watch(() => formData.value.jobSelected, (newValue) => {
  const selectedJobOption = jobOptions.value.find(option => option.label === newValue);
  if (selectedJobOption) {
    formData.value.jobSelectedId = selectedJobOption.id;
...
// Escuchar cambios en formData.jobSelected y actualizar formData.jobSelectedId
watch(() => formData.value.sexSelected, (newValue) => {
  const selectedJobOption = sexOptions.find(option => option.label === newValue);
  if (selectedJobOption) {
...
// Escuchar cambios en formData.jobSelected y actualizar formData.jobSelectedId
watch(() => formData.value.statusSelected, (newValue) => {
  const selectedJobOption = statusOptions.find(option => option.label === newValue);
  if (selectedJobOption) {
