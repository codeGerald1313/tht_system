    ...
    <Card noborder>
      <div class="md:flex justify-between pb-6 md:space-y-0 space-y-3 items-center">
        ...
        <div class="flex items-center space-x-3"> <!-- Contenedor flex para alinear los elementos -->
          ...
          <InputGroup v-model="searchTerm" placeholder="Buscar Empleado" type="text" prependIcon="heroicons-outline:search"
            merged />
        </div>
      </div>
      <vue-good-table class="-mx-6" :columns="columns"
        styleClass=" vgt-table  table-head   v-middle striped  listview" :rows="projects"
        :pagination-options="{
              enabled: true,
              perPage: perpage,
            }" :search-options="{
        ...
        <template v-slot:table-row="props">
          ...
          <span v-if="props.column.field == 'company_job.description'" class="font-bold">
            {{ props.row.company_job.description }}
          </span>
          <span v-if="props.column.field == 'cellphone'">
            {{ props.row.cellphone }}
          </span>
      ...
      <EditProject :activeModal="showEditModal" @close="showEditModal = false" :employeeData="employeeData" 
        title="Actualizar Datos Empleado"  @updateEmployeeList="listarEmpleados"></EditProject>


        <!--<AddProject @updateEmployeeList="listarEmpleados"></AddProject> -->
...
import { useToast } from 'vue-toastification'; // Importa useToast desde vue-toastification
...
export default {
  ...
  data() {
    return {
      ...
      current: 1,
      perpage: 6,
      employeeData: {}, // Objeto para almacenar los datos del empleado
      pageRange: 10,
        ...
        },
        {
          ...
          doit: async (data) => {
            try {
              ...
              await axios.delete(`${import.meta.env.VITE_API_URL}/employees/delete/${data.id}`, headers);
  ...
  methods: {
    ...
    async listarEmpleados() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/employees/list`, headers);
        this.projects = response.data.data;

        const toast = useToast();
        toast.success('Empleados listados correctamente', {
          timeout: 1500, // Cierre automático después de 1.5 segundos
        });
