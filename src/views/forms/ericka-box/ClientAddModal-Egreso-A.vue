<template>
  <div>
    <Modal :activeModal="store.addmodal3" @close="store.closeModal3" title="Registrar Nuevo Egreso - Caja Ericka" centered>
      <form @submit.prevent="addProject" class="space-y-4">
        ...
        <div class="grid lg:grid-cols-2 gap-4 grid-cols-1">
          <FromGroup name="d3">
            <Select label="Receptor (opcional)" :options="userposition" v-model="selected" class="emisor-select" />
          </FromGroup>
          <FromGroup name="d4">
...
const saveIngreso = () => {
  ...
  axios.post(`${import.meta.env.VITE_API_URL}/moneys-ericka/create-expense`, income.value, {
    ...headers
  })
    .then(response => {
