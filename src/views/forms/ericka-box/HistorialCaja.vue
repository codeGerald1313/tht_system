          ...
          <div>
            <vue-good-table :columns="columns" styleClass=" vgt-table  lesspadding2 centered " :rows="advancedTable"
              ...
              <template v-slot:table-row="props">
                ...
                <span v-if="props.column.field == 'amount_opening'" class="font-medium text-blue-500">
                  S/. {{ props.row.amount_opening }}
                </span>

                <span v-if="props.column.field == 'amount_closing'" class="font-medium text-purple-500">
...
export default {
  ...
  methods: {
    handleVerDocumento(id) {
      axios.get(`${import.meta.env.VITE_API_URL}/list-box-cuadre-ericka/${id}`)
        .then(response => {
    ...
    fetchMoneyBoxes() {
      axios.get(`${import.meta.env.VITE_API_URL}/moneyboxes-ericka/list-normally`, headers)
        .then(response => {
