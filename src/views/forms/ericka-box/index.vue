<template>
  ...
  <div>
    <div class="flex justify-between flex-wrap items-center">
      ...
      <TransferirCajaModalPrincipal :id="id" @guardadoExitoso="handleGuardadoExitosoTransferCajaPrincipal" />
...
import TransferirCajaModalPrincipal from "./TranfserModalAddPrincipal";
...
export default {
  ...
  methods: {
    ...
    handleVerDocumento() {
      axios.get(`${import.meta.env.VITE_API_URL}/list-box-cuadre-ericka/${this.id}`)
        .then(response => {
    ...
    async fetchSummaryPaymethods(id) {
      try {
        ...
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/moneyboxes-ericka/list/${id}`, headers); // Reemplaza 'URL_DEL_BACKEND' con la URL correcta de tu backend y '${id}' con el ID que deseas obtener
    ...
    async fetchSummaryPaymethodsInit() {
      try {
        // Llamar al backend para obtener los datos
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/moneyboxes-ericka/list`, headers); // Reemplaza 'URL_DEL_BACKEND' con la URL correcta de tu backend y '${id}' con el ID que deseas obtener
  ...
  mounted() {
    ...
    axios.get(`${import.meta.env.VITE_API_URL}/moneyboxes-ericka/last-id-with-other-owner-ericka`, headers)
      .then(response => {
