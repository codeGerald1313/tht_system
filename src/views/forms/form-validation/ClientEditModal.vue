    ...
    <Modal :activeModal="props.activeModal" @close="closeEditModal" title="Actualizar Registro de Egreso" centered>
      <form @submit.prevent="addProject" class="space-y-4">
        <div class="grid lg:grid-cols-2 gap-4 grid-cols-1">
          ...
          <FromGroup label="Descripción egreso" name="d2">
            <Textinput type="text" placeholder="Descripción egreso" v-model.trim="income.gloss" />
        ...
        <div class="grid lg:grid-cols-2 gap-4 grid-cols-1">
          ...
          <FromGroup name="d4">
            <Select label="Concepto egreso" :options="concepts" v-model="conceptSelected" class="concept-select"
        ...
        <div class="grid lg:grid-cols-1 gap-4 grid-cols-1">
          <div v-if="selected === 'clients'" class="flex items-center">
            <FromGroup label="Clientes" class="flex-1">
              <Select :options="customerOptions" v-model="selectedCustomer" class="client-select" />
            </FromGroup>
          ...
          <Checkbox label="Incluye Comprobante" v-model="includeReceipt" :checked="includeReceipt" />

          <template v-if="includeReceipt">
            <div class="grid lg:grid-cols-2 gap-4 grid-cols-1">
              <FromGroup label="Tipo de Comprobante">
                <Select :options="recipentTypDocument" v-model="typeDocumentSelected" />
              </FromGroup>
              <FromGroup label="N° de Comprobante">
                <Textinput type="text" v-model="income.nro_voucher_external" placeholder="Ingrese N° de Comprobante" />
              </FromGroup>
            </div>
          </template>
      ...
      dateFormat: 'Y-m-d',
        ...
        <div class="grid lg:grid-cols-2 gap-4 grid-cols-1">
          <FromGroup name="d5" class="mt-1">
            ...
            <Select id="payment-type" :options="paymentMethods" disabled v-model="selectedPaymentType"
...
const showBlock = ref(false); // Define showBlock como una variable reactiva
...
const props = defineProps({
  activeModal: Boolean,
  egresoData: Object // Se espera que la data del empleado venga como objeto
})

const emits = defineEmits(['close', 'updateEgresoList']);
...
const fetchConceptsIncome = async () => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/moneys/expense-concepts`, headers); // Cambia la ruta '/api/departments' por la ruta real de tu API
...
const fetchTypeComprobante = async () => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/typevouchers/list`, headers); // Cambia la ruta '/api/departments' por la ruta real de tu API
    recipentTypDocument.value = response.data.data.map(document => ({
      value: document.id, // Cambiamos id por value
      label: document.description // Cambiamos name por label
    }));
...
const typeDocumentSelected = ref(null);
...
const saveIngreso = () => {
  ...
  income.value.voucher_external = typeDocumentSelected.value;
  // Asigna el valor de includeReceipt.value a income.value.is_voucher_external
  income.value.is_voucher_external = includeReceipt.value;

  // Si includeReceipt.value es false, asigna null a income.value.voucher_external
  if (!includeReceipt.value) {
    income.value.nro_voucher_external = null;
...
watch(() => props.egresoData, (newData) => {
  if (newData) {
    // Primero, reiniciamos todos los valores seleccionados
    selectedCustomer.value = null;
    selectedEmployee.value = null;
    selectedProvider.value = null;

    // Luego, verificamos qué tipo de dato se recibió y establecemos el valor correspondiente
    if (newData.client_id) {
      selected.value = 'clients'; // Esto garantiza que el selector cambie automáticamente
      selectedCustomer.value = newData.client_id;
    } else if (newData.employee_id) {
      selected.value = 'employees';
      selectedEmployee.value = newData.employee_id;
    } else if (newData.provider_id) {
      selected.value = 'providers';
      selectedProvider.value = newData.provider_id;
    }

    // Actualizamos cualquier otro valor necesario
    income.value.id = newData.id;
    income.value.gloss = newData.gloss;
    income.value.nro_voucher_external = newData.nro_voucher_external;
    ...
    income.value.amount = newData.amount;
    includeReceipt.value = newData.is_voucher_external === 1 ? true : false;
