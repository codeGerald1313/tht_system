<template>
  <div>
    <Card noborder>
        ...
        <template v-slot:table-row="props">
          ...
          <span v-if="props.column.field == 'nro_voucher'">
            <div class="flex space-x-3 items-center text-left rtl:space-x-reverse">
              <span class="font-bold text-purple-700">{{ props.row.nro_voucher }}</span>
            </div>
          </span>
      ...
      <EditProject :activeModal="showEditModal" @close="showEditModal = false" :egresoData="egresoData"
        title="Actualizar Datos Ingreso" @updateEgresoList="listarIngresos"></EditProject>
  ...
  methods: {
    async reloadCrmTable() {
      try {
        ...
        console.error('Error al recargar Egresos :', error);
      }
    },
    async listarIngresos() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/moneys/list-expense`, {
          params: {
        ...
        if (this.projects && this.projects.length > 0) {
          // Mostrar un toast de éxito indicando que los egresos fueron listados correctamente
          const toast = useToast();
          toast.success('Egresos listados correctamente', {
            timeout: 1500, // Cierre automático después de 1.5 segundos
          });
        } else {
          // Mostrar un toast informativo indicando que no se encontraron egresos para la fecha actual
          const toast = useToast();
          toast.info('Para el día de hoy, todavía no exiten registros de egreso', {
            timeout: 2500, // Cierre automático después de 1.5 segundos
          });
        }
      } catch (error) {
        console.error('Error al obtener los datos de los egresos:', error);

        // En caso de error, mostrar un toast de error
        const toast = useToast();
        toast.error('Ocurrió un error al obtener los egresos. Por favor, inténtalo de nuevo.', {
          timeout: 1500, // Cierre automático después de 1.5 segundos
        });
      }
    },
    async downloadFile2() {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/export-money-por-dia-expense`,
          {
            params: {
              date: this.dateDefault
            },
    ...
    async dateDefault(newValue, oldValue) {
      try {
        ...
        if (this.projects && this.projects.length > 0) {
          ...
          // Mostrar un toast informativo indicando que no se registraron egresos para el día de hoy
          const toast = useToast();
          toast.info('Para este día, no hay registros de egreso', {
            timeout: 2500, // Cierre automático después de 1.5 segundos
          });
        }
      } catch (error) {
