<template>
  <div>
    <Modal :activeModal="props.activeModal" @close="closeEditModal" title="Actualizar Registro de Ingreso" centered>
...
const props = defineProps({
  activeModal: Boolean,
  ingresoData: Object // Se espera que la data del empleado venga como objeto
})

const emits = defineEmits(['close', 'updateIngresoList']);
...
const saveIngreso = () => {
  ...
  axios.post(`${import.meta.env.VITE_API_URL}/moneys/edit/${income.value.id}`, income.value, {
    ...
    .then(response => {
      ...
      emits('updateIngresoList'); // Emitir el evento personalizado al componente padre
      closeEditModal();
...
watch(() => props.ingresoData, (newData) => {
  if (newData) {
